...
export default class LoginDBClient {
    ...
    async updateOne(beforeData: Partial<LoginInfo>, afterData: Partial<LoginInfo>): Promise<number> {
        const result = await this.update(beforeData, afterData);
        if ( result !== 1) {
            throw new Error(`updateOne must have one affectedRows, but result is ${result}`);
        }
        return result;
    }
    ...
    async deleteOne(deleteData: Partial<LoginInfo>): Promise<number> {
        const result = await this.delete(deleteData);
        if ( result !== 1) {
            throw new Error(`deleteOne must have one affectedRows, but result is ${result}`);
        }
        return result;
    }
